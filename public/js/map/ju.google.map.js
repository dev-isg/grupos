...
(function( $ ) { // JQuery wrapper
    ...
    JuGoogleMap.prototype.init = function() 
    {
    	...
    	else if (this.options.editable && !this.marker) 
    	{
    		...
    		this.refreshLocation(false, function(){
    			if (!self.marker)
    			{
    				self.setMarker(self.location.getPosition().lat(), 
    						self.location.getPosition().lng(), 
    					null, null);
    				self.map.setCenter(self.location.getPosition());
    ...
    JuGoogleMap.prototype.getBtnLocator = function()
    {
    	...
    	if (!this.btnLocator)
    	{
    		this.btnLocator = $("<button type='button' class='btn btn-inverse locator-view'/>")
    ...
    JuGoogleMap.prototype.getBtnCenterMarker = function()
    {
    	...
    	{
    		this.btnCenterMarker = $("<button type='button' class='btn btn-inverse center-view'/>")
    ...
    JuGoogleMap.prototype.getBtnRoute = function()
    {
    	...
    	if (!this.btnRoute)
    	{
    		this.btnRoute = $("<button type='button' class='btn btn-inverse route-map'/>")
    ...
    JuGoogleMap.prototype.setMarker = function(lat, lng, address, addressRef)
    {
    	...
    	if (!this.marker)
		{
			...
			google.maps.event.addListener(this.marker, 'dragend', function(){
				self.setMarker(self.marker.getPosition().lat(), self.marker.getPosition().lng(), null, self.marker.addressRef);
			});
		...
		{
			if (this.marker.getPosition().lat() != lat || this.marker.getPosition().lng() != lng)
    ...
    JuGoogleMap.prototype.updateMarkerAddress = function()
    {
		...
		this.getGeoCoder().geocode({'latLng': this.marker.position}, function(results, status) 
		{
			...
			{
				self.setMarker(self.marker.getPosition().lat(), self.marker.getPosition().lng(), 
    ...
    {
    	if (!this.options.dataBound) return;
    	var lat = (this.marker) ? this.marker.getPosition().lat() : null;
    	var lng = (this.marker) ? this.marker.getPosition().lng() : null;
    ...
    JuGoogleMap.prototype._populateToDataBound = function()
    {
    	if (!this.options.dataBound) return;	
    	if (this.options.dataBound.lat)
    		$(this.options.dataBound.lat).val(this.marker?this.marker.getPosition().lat():"");
    	if (this.options.dataBound.lng)
    		$(this.options.dataBound.lng).val(this.marker?this.marker.getPosition().lng():"");
    	if (this.options.dataBound.address)
    ...
    JuGoogleMap.prototype._onMapClicked = function(pos)
    {
    	if (this.options.editable && this.options.clickSetsMarker)
    		this.setMarker(pos.latLng.lat(), pos.latLng.lng(), null, (this.marker) ? this.marker.addressRef : null);
    }
